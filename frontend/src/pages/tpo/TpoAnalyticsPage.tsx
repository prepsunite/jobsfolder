...
export default function TpoAnalyticsPage() {
  ...
  // 3. Fetch College Students for live roster correlation & roll number guarantee
  ...
  const resolveStudent = (att: StudentExamAttempt) => {
    ...
    const department =
      s.department && s.department !== 'General' && s.department !== 'CSE'
        ? s.department
        : (matched?.department || s.department || 'General');
                    ...
                    <tr key={exam.id} className="hover:bg-slate-50/60 dark:hover:bg-slate-800/30 transition-colors">
                      ...
                      <td className="py-4 px-4">
                        {(() => {
                          ...
                          if (timing === 'UPCOMING') {
                            return (
                              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-blue-50 text-blue-700 dark:bg-blue-950/40 dark:text-blue-400 border border-blue-200 dark:border-blue-800">
                                Scheduled
                              </span>
                            );
                          }
                          if (timing === 'CONCLUDED') {
                            return (
                              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold bg-slate-100 text-slate-600 dark:bg-slate-800 dark:text-slate-400 border border-slate-200 dark:border-slate-700">
                                Concluded
                              </span>
                            );
                          }
