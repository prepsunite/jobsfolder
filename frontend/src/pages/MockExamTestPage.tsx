      ...
      <div className="fixed inset-0 z-40 bg-gray-100 dark:bg-[#0f1012] text-gray-900 dark:text-white flex flex-col select-none overflow-hidden">
        ...
        <div className="sticky top-0 z-20 bg-white dark:bg-[#151618] border-b border-gray-200 dark:border-[#25262a] px-3 sm:px-4 py-2.5 sm:py-3 flex items-center justify-between shadow-sm gap-2">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0 shrink-0">
            {/* Toggle Left Question Palette Button */}
            <button
              type="button"
              ...
              className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl border text-xs font-bold transition-all cursor-pointer ${
                isPaletteOpen
                  ? 'bg-gray-100 dark:bg-[#202225] border-gray-300 dark:border-[#383a40] text-gray-900 dark:text-white'
                  : 'bg-[#FD4A32]/10 border-[#FD4A32]/30 text-[#FD4A32] hover:bg-[#FD4A32]/20'
              }`}
              title={isPaletteOpen ? 'Collapse Left Question Palette' : 'Show Left Question Palette'}
            >
              <LayoutGrid className="w-3.5 h-3.5" />
              <span className="text-[11px]">{isPaletteOpen ? 'Hide Grid' : 'Question Grid'}</span>
            </button>
            ...
            <div className="min-w-0">
              ...
              <h2 className="text-xs sm:text-sm font-bold text-gray-900 dark:text-white truncate max-w-[140px] sm:max-w-[220px]">
                {exam?.title}
              </h2>
            </div>
          </div>
        ...
        {/* Test Body: Question Palette (Left) + Main Question Card Area (Right) */}
        <div className="flex-1 flex flex-col md:flex-row overflow-hidden relative min-h-0">
          
          {/* 1. Left Column: Question Status Palette & Question Numbers Grid */}
          {isPaletteOpen && (
            <div className="w-full md:w-80 shrink-0 bg-white dark:bg-[#151618] border-b md:border-b-0 md:border-r border-gray-200 dark:border-[#25262a] p-4 sm:p-5 flex flex-col gap-4 overflow-y-auto z-10">
              
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="text-xs font-black uppercase tracking-wider text-gray-800 dark:text-gray-200 flex items-center gap-1.5">
                    <LayoutGrid className="w-4 h-4 text-[#FD4A32]" />
                    Question Palette
                  </h4>
                ...
                <button
                  ...
                  className="hidden md:flex p-1.5 text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 rounded-lg hover:bg-gray-100 dark:hover:bg-[#202225] transition-colors cursor-pointer"
                  title="Collapse Palette (Gain Full Screen for Question)"
                >
                ...
                return (
                  <div className="grid grid-cols-2 gap-2 text-[10px] font-semibold text-gray-600 dark:text-gray-300 bg-gray-50 dark:bg-[#1c1d20] p-3 rounded-xl border border-gray-200 dark:border-[#2b2d31]">
                    <div className="flex items-center justify-between gap-1">
                      ...
                      <strong className="font-mono text-emerald-600 dark:text-emerald-400">{secAns}</strong>
                    </div>
                    <div className="flex items-center justify-between gap-1">
                      <span className="flex items-center gap-1.5">
                      ...
                      <strong className="font-mono text-purple-600 dark:text-purple-400">{secMarked}</strong>
                    </div>
                    <div className="flex items-center justify-between gap-1">
                      <span className="flex items-center gap-1.5">
                      ...
                      <strong className="font-mono text-orange-600 dark:text-orange-400">{secNotAnswered}</strong>
                    </div>
                    <div className="flex items-center justify-between gap-1">
                      <span className="flex items-center gap-1.5">
                        <span className="w-2.5 h-2.5 rounded bg-gray-200 dark:bg-[#34373d] shrink-0" />
                        <span>Not Visited</span>
                      </span>
                      <strong className="font-mono text-gray-500 dark:text-gray-400">{secNotVisited}</strong>
                    </div>
                  </div>
                );
              })()}

              {/* Question Numbers Grid (1 to N) */}
              <div className="space-y-1.5">
                ...
                <div className="grid grid-cols-5 gap-2 overflow-y-auto max-h-72 sm:max-h-80 p-1 pr-1.5">
                  {currentSectionQIds.map((qId, idx) => {
                    ...
                    if (isMarked) {
                      colorClasses = 'bg-purple-600 text-white font-bold shadow-xs';
                    } else if (isAnswered) {
                      colorClasses = 'bg-emerald-500 text-white font-bold shadow-xs';
                    } else if (isVisited) {
                      colorClasses = 'bg-orange-500 text-white font-bold shadow-xs';
                    }
          ...
          {/* Collapsed Palette Quick Expand Handle (Desktop) */}
          {!isPaletteOpen && (
            <div className="hidden md:flex flex-col items-center py-4 px-1.5 border-r border-gray-200 dark:border-[#25262a] bg-white dark:bg-[#151618] shrink-0">
              <button
                type="button"
                onClick={() => setIsPaletteOpen(true)}
                className="p-2 rounded-xl bg-gray-100 dark:bg-[#202225] hover:bg-[#FD4A32] hover:text-white text-gray-700 dark:text-gray-300 transition-all cursor-pointer shadow-xs"
                title="Expand Question Palette"
              >
                <ChevronRight className="w-4 h-4" />
              </button>
              <span className="[writing-mode:vertical-rl] text-[10px] font-bold uppercase tracking-wider text-gray-400 mt-4 rotate-180">
                Question Palette
              </span>
            </div>
          )}
